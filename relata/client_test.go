...
func TestQuery_MatcherHint(t *testing.T) {
	var gotBody map[string]any
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		b, _ := io.ReadAll(r.Body)
		_ = json.Unmarshal(b, &gotBody)
		w.Header().Set("Content-Type", "application/json")
		fmt.Fprint(w, `{"query_id":"q-1","elapsed_ms":1,"rows":[]}`)
	}))
	defer srv.Close()

	const cypher = "MATCH (a:Person)-[:KNOWS]->(b:Person)-[:KNOWS]->(c:Person) RETURN a, b, c"
	c := newTestClient(srv, &ClientOptions{DefaultPurpose: "analytics"})
	...
	cases := []struct {
		...
		{"vf3", WithMatcher(MatcherVf3), "/*+ matcher=vf3 */ " + cypher},
		{"bfs", WithMatcher(MatcherBfs), "/*+ matcher=bfs */ " + cypher},
		{"auto", WithMatcher(MatcherAuto), cypher},
		{"default", nil, cypher},
	}
	for _, tc := range cases {
		...
		if _, err := c.Query(context.Background(), cypher, opts...); err != nil {
			t.Fatalf("%s: Query: %v", tc.name, err)
		}
		if gotBody["sql"] != tc.want {
			t.Fatalf("%s: sql = %v, want %q", tc.name, gotBody["sql"], tc.want)
		}
	}

	if _, err := c.Query(context.Background(), cypher, WithMatcher(Matcher("gremlin"))); err == nil {
		t.Fatal("expected error for unknown matcher, got nil")
	}
}

func TestQueryWithParams_MatcherHint(t *testing.T) {
	var gotBody map[string]any
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		b, _ := io.ReadAll(r.Body)
		_ = json.Unmarshal(b, &gotBody)
	...
	const cypher = "MATCH (a:Person {id: $1})-[:KNOWS]->(b:Person) RETURN a, b"
	c := newTestClient(srv, &ClientOptions{DefaultPurpose: "analytics"})
	if _, err := c.QueryWithParams(context.Background(), cypher, []any{"p1"}, WithMatcher(MatcherVf3)); err != nil {
		t.Fatalf("QueryWithParams: %v", err)
	}
	if gotBody["sql"] != "/*+ matcher=vf3 */ "+cypher {
